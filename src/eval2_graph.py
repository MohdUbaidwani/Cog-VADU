...
from src.utils.plot_utils import plot_scores

import matplotlib
matplotlib.use('Agg')  # Force headless PNG saving for SLURM—no GUI needed
...
#After video_scores and video_labels are ready:
        print(f"[DEBUG] Processing video: {safe_name}, scores shape: {np.array(video_scores).shape}, labels shape: {np.array(video_labels).shape if not without_labels else 'N/A'}")
        print(f"[DEBUG] Output dir exists: {output_dir.exists()}, writable: {output_dir.is_dir() and os.access(output_dir, os.W_OK)}")  # Add import os at top if needed
        plot_scores(
        scores=np.array(video_scores),
        labels=np.array(video_labels),
        video_name=safe_name,
        save_dir=output_dir,
        normal_id=normal_label
    )
